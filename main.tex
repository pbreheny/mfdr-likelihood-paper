...
%% I'm just cutting MCP from this section for now...I would like to include it, but we don't have enough details to make the results interesting at this point.

%% NOTE: Should we show MCP in this figure too?
...
Using a target 10\% mFDR, we select 9 genes associated with survival.  For comparison we analyze the data using the approach proposed by \citet{Meinshausen2009}, which repeatedly performs sample splitting using a large number random splits. Using this approach we are unable to select any genes.  As mentioned earlier, the {\tt covTest} package does not (currently) accommodate survival data.

%%leads to a choice of $\lambda$ that selects 9 genes. Using cross validation results in the selection of a much smaller $\lambda$ and selects of 28 genes.  However, the estimated mFDR at this choice of $\lambda$ is 100\% -- i.e., the expected number of noise features exceeds the number of actual variables selected -- indicating that many of these selections are likely to be noise variables.  Using the MCP penalty, the targeted 10\% mFDR threshold results in the selection of only 2 genes, reflecting the tendency of MCP to lead to more parsimonious models.

%% NOTE: We need to discuss cross-validation here...I'm not convinced these results are correct.  You write that MCP+CV selects 26 genes.  When I tried it, I found that MCP+CV selects 26 features, but 18 were baseline covariates, only 8 were genetic.  Striking CV from here for now, but would like to include in next version.

%% NOTE: Do we need both approaches here?  I think only the adjusted one is relevant.
...
0.2041 & 0.00 &   0 & 0.00 & 0.00 &   0 & 0.00 \\ 
  ...
  0.1336 & 3.10 &  13 & 0.24 & 2.69 &   7 & 0.38 \\ 
   \hline
\end{tabular}
 \caption{\label{Tab:spira} Estimated mFDR results for the Spira data at 15 values of the $\lambda$ sequence which straddle our target mFDR.  EF=Estimated number of False discoveries, S=number of selected genes.}
\end{table}
...
%% NOTE: Would be nice to mention time here (or in previous section): How long do each of these methods take to run?
