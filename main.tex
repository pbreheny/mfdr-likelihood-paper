...
For comparison we also analyzed the data using repeated sample splitting with 100 random splits, as advocated by  \citet{Meinshausen2009}. However, this approach was unable to select any genes at a false discovery rate of 10\%.  In fact, none of the genetic features were selected in more than half of the stage one splits, thus even allowing for more liberal false discovery rates would not result in any selections. As mentioned earlier, the {\tt covTest} package does not (currently) accommodate survival data.
%% All genetic features have median p-values of 1 even before adjusting for fdr, only clinical features have median p-values less than 1

%\textcolor{red}{of 10\%?  20\%?  It would be nice to say something here like, even if we went to 50\%, it still couldn't select anything}.
